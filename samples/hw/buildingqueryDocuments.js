    ...
    MongoClient.connect('mongodb://localhost:27017/crunchbase', function(err, db) {
        ...
        cursor.forEach(
            function(doc) {
                ...
                //if its already here, skip
                if (doc.permalink in companiesSeen) return;
                //add the doc to the hash with permaLink as key
...
function queryDocument(options) {
    ...
    if (("firstYear" in options) && ("lastYear" in options)) {
      ...
      //query.founded_year = {"founded_year":{ "$gte": options.firstYear },"founded_year":{ "$lte": options.lastYear }   }
      query.founded_year =  { "$gte": options.firstYear , "$lte": options.lastYear }
       
     //  console.log("XXX"+JSON.stringify(query.founded_year)+"XXX")
    ...
    if ("city" in options) {
        ...
        */
        query["offices.city"] =  {"$in": [options.city]}
    }
